...
class YOLOXBox(object):
    ...
    def __call__(self, yolo_head_out, anchors, im_shape, scale_factor):
        ...
        origin_shape = paddle.cast(origin_shape, 'int32')
        for i, head_out in enumerate(yolo_head_out):
            boxes, scores = self.yolox_box(
                head_out, origin_shape, 
                anchors[i], self.num_classes,
    ...
    def yolox_box(self, feat, origin_shape, anchors, num_classes, downsample, clip_bbox, scale_x_y):
        b = paddle.shape(feat)[0]
        c, h, w = feat.shape[1:]
        n = 5 + self.point_num * 2 + num_classes
        anchors_num = int(c / n)
        feat = feat.transpose((0, 2, 3, 1)).reshape((b, h, w, anchors_num, int(c / anchors_num)))
        feat = feat.transpose((0, 3, 1, 2, 4)).reshape((b, h * w * anchors_num, int(c / anchors_num)))
        yv, xv = paddle.meshgrid([paddle.arange(h), paddle.arange(w)])
        grid = paddle.cast(paddle.stack((xv, yv, xv, yv, xv, yv), 2), dtype=feat.dtype)
        grid = grid.reshape((1, h, w, anchors_num, 2)).transpose((0, 3, 1, 2, 4))
        grid = grid.reshape((1, h * w * anchors_num, 2))
        pxy = (scale_x_y * F.sigmoid(feat[:, :, 0:2]) - (0.5 * (scale_x_y - 1.)) + grid)
        pw = paddle.exp(feat[:, :, 2:3])
        ph = paddle.exp(feat[:, :, 3:4])
        pw[:, 0:h*w,:] = paddle.exp(feat[:, 0:h*w, 2:3]) * anchors[0] / downsample
        ph[:, 0:h*w,:] = paddle.exp(feat[:, 0:h*w, 3:4]) * anchors[1] / downsample
        pw[:, h*w:2*h*w,:] = paddle.exp(feat[:, h*w:2*h*w, 2:3]) * anchors[2] / downsample
        ph[:, h*w:2*h*w,:] = paddle.exp(feat[:, h*w:2*h*w, 3:4]) * anchors[3] / downsample
        pw[:, 2*h*w:3*h*w,:] = paddle.exp(feat[:, 2*h*w:3*h*w, 2:3]) * anchors[4] / downsample
        ph[:, 2*h*w:3*h*w,:] = paddle.exp(feat[:, 2*h*w:3*h*w, 3:4]) * anchors[5] / downsample
        px1y1 = feat[:, :, 0:2]
        px2y2 = feat[:, :, 0:2]
        px1y1[:, :, 0:1] = pxy[:, :, 0:1] - pw / 2.0
        px1y1[:, :, 1:2] = pxy[:, :, 1:2] - ph / 2.0
        px2y2[:, :, 0:1] = pxy[:, :, 0:1] + pw / 2.0
        px2y2[:, :, 1:2] = pxy[:, :, 1:2] + ph / 2.0
...
class YOLOPolyBox(object):
    ...
    def __call__(self, yolo_head_out, anchors, im_shape, scale_factor):
        ...
        origin_shape = paddle.cast(origin_shape, 'int32')
        for i, head_out in enumerate(yolo_head_out):
            boxes, scores = self.yolopoly_box(
                head_out, origin_shape, anchors[i], self.num_classes,
                self.downsample_ratio // 2**i, self.clip_bbox, self.scale_x_y)
            boxes_list.append(boxes)
            scores_list.append(paddle.transpose(scores, perm=[0, 2, 1]))
        yolo_boxes = paddle.concat(boxes_list, axis=1)
    ...
    def yolopoly_box(self, feat, origin_shape, anchors, num_classes, downsample, clip_bbox, scale_x_y):
        b = paddle.shape(feat)[0]
        c, h, w = feat.shape[1:]
        n = 3 + self.point_num * 2 + num_classes
        anchors_num = int(c / n)
        ...
        pw1 = F.sigmoid(feat[:, :, 2:3])
        ph1 = F.sigmoid(feat[:, :, 3:4])
        pw1[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 2:3]) - 0.5) * 4 * anchors[0] / downsample
        ph1[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 3:4]) - 0.5) * 4 * anchors[1] / downsample
        pw1[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 2:3]) - 0.5) * 4 * anchors[2] / downsample
        ph1[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 3:4]) - 0.5) * 4 * anchors[3] / downsample
        pw1[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 2:3]) - 0.5) * 4 * anchors[4] / downsample
        ph1[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 3:4]) - 0.5) * 4 * anchors[5] / downsample
        pw2 = feat[:, :, 4:5]
        ph2 = feat[:, :, 5:6]
        pw2[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 4:5]) - 0.5) * 4 * anchors[0] / downsample
        ph2[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 5:6]) - 0.5) * 4 * anchors[1] / downsample
        pw2[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 4:5]) - 0.5) * 4 * anchors[2] / downsample
        ph2[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 5:6]) - 0.5) * 4 * anchors[3] / downsample
        pw2[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 4:5]) - 0.5) * 4 * anchors[4] / downsample
        ph2[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 5:6]) - 0.5) * 4 * anchors[5] / downsample
        pw3 = feat[:, :, 6:7]
        ph3 = feat[:, :, 7:8]
        pw3[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 6:7]) - 0.5) * 4 * anchors[0] / downsample
        ph3[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 7:8]) - 0.5) * 4 * anchors[1] / downsample
        pw3[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 6:7]) - 0.5) * 4 * anchors[2] / downsample
        ph3[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 7:8]) - 0.5) * 4 * anchors[3] / downsample
        pw3[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 6:7]) - 0.5) * 4 * anchors[4] / downsample
        ph3[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 7:8]) - 0.5) * 4 * anchors[5] / downsample
        pw4 = feat[:, :, 8:9]
        ph4 = feat[:, :, 9:10]
        pw4[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 8:9]) - 0.5) * 4 * anchors[0] / downsample
        ph4[:, 0:h*w,:] = (F.sigmoid(feat[:, 0:h*w, 9:10]) - 0.5) * 4 * anchors[1] / downsample
        pw4[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 8:9]) - 0.5) * 4 * anchors[2] / downsample
        ph4[:, h*w:2*h*w,:] = (F.sigmoid(feat[:, h*w:2*h*w, 9:10]) - 0.5) * 4 * anchors[3] / downsample
        pw4[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 8:9]) - 0.5) * 4 * anchors[4] / downsample
        ph4[:, 2*h*w:3*h*w,:] = (F.sigmoid(feat[:, 2*h*w:3*h*w, 9:10]) - 0.5) * 4 * anchors[5] / downsample

        px1y1 = feat[:, :, 0:2]
        px2y2 = feat[:, :, 0:2]
        px3y3 = feat[:, :, 0:2]
        px4y4 = feat[:, :, 0:2]
        px1y1[:, :, 0:1] = pxy[:, :, 0:1] + pw1
        px1y1[:, :, 1:2] = pxy[:, :, 1:2] + ph1
        px2y2[:, :, 0:1] = pxy[:, :, 0:1] + pw2
        px2y2[:, :, 1:2] = pxy[:, :, 1:2] + ph2
        px3y3[:, :, 0:1] = pxy[:, :, 0:1] + pw3
        px3y3[:, :, 1:2] = pxy[:, :, 1:2] + ph3
        px4y4[:, :, 0:1] = pxy[:, :, 0:1] + pw4
        px4y4[:, :, 1:2] = pxy[:, :, 1:2] + ph4
        ...
        if self.clip_bbox:
            ...
            px2 = paddle.clip(px2y2[:, :, 0:1], min=0., max=w)
            py2 = paddle.clip(px2y2[:, :, 1:2], min=0., max=h)
            px3 = paddle.clip(px3y3[:, :, 0:1], min=0., max=w)
            py3 = paddle.clip(px3y3[:, :, 1:2], min=0., max=h)
            px4 = paddle.clip(px4y4[:, :, 0:1], min=0., max=w)
            py4 = paddle.clip(px4y4[:, :, 1:2], min=0., max=h)
        ...
        polyes = paddle.concat([x1, y1, x2, y2, x3, y3, x4, y4], axis=-1)
        scores = F.sigmoid(feat[:, :, 11:]) * F.sigmoid(feat[:, :, 10:11])
        return polyes, scores
