...
class YOLOv3Poly(BaseArch):
    ...
    def __init__(self,
        ...
        """
        super(YOLOv3Poly, self).__init__(data_format=data_format)
    ...
    def _forward(self):
        ...
        if self.training:
            ...
            if self.for_mot:
                boxes_idx, poly, bbox_num, nms_keep_idx = self.post_process(
                ...
                if self.return_idx:
                    _, poly, bbox_num, _ = self.post_process(
                        yolo_head_outs, self.yolo_head.mask_anchors)
                else:
                    poly, bbox_num = self.post_process(
                        yolo_head_outs, self.yolo_head.mask_anchors,
                        self.inputs['im_shape'], self.inputs['scale_factor'])
                output = {'poly': poly, 'bbox_num': bbox_num}
