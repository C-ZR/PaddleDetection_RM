...
def xywh2xyxy_poly(poly):
    x, y, w1, h1, w2, h2, w3, h3, w4, h4 = poly
    x1 = x - (w1 + w4) * 0.5
    y1 = y - (h1 + h2) * 0.5
    x2 = x + (w2 + w3) * 0.5
    y2 = y + (h3 + h4) * 0.5
...
def decode_poly_yolo(box, anchor, downsample_ratio):
    """decode yolo box

    Args:
        poly (list): [x, y, w, h], all have the shape [b, na, h, w, 1]
        anchor (list): anchor with the shape [na, 2]
        downsample_ratio (int): downsample ratio, default 32
        scale (float): scale, default 1.

    Return:
        poly (list): decoded poly, [x, y, w, h], all have the shape [b, na, h, w, 1]
    """
    x, y, w, h = box
    na, grid_h, grid_w = x.shape[1:4]
    grid = make_grid(grid_h, grid_w, x.dtype).reshape((1, 1, grid_h, grid_w, 2))
    ...
    anchor = paddle.cast(anchor, x.dtype)
    anchor = anchor.reshape((1, na, 1, 1, 2))
    w1 = w * 4 * anchor[:, :, :, :, 0:1] / (downsample_ratio * grid_w)
    h1 = h * 4 * anchor[:, :, :, :, 1:2] / (downsample_ratio * grid_h)
