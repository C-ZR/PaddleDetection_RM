...
class YOLOPolyHead(nn.Layer):
    ...
    def __init__(self,
        ...
        Args:
            ...
            data_format (str): data format, NCHW or NHWC
        """
        super(YOLOPolyHead, self).__init__()
        assert len(in_channels) > 0, "in_channels length should > 0"
        self.in_channels = in_channels
        self.num_classes = num_classes
        self.loss = loss
        ...
        for i in range(len(self.anchors)):
            ...
            if self.iou_aware:
                num_filters = len(self.anchors[i]) * (self.num_classes + 4 + self.point_num * 2)
            else:
                num_filters = len(self.anchors[i]) * (self.num_classes + 3 + self.point_num * 2)
            name = 'yolo_output.{}'.format(i)
            conv = nn.Conv2D(
                in_channels=self.in_channels[i],
                out_channels=num_filters,
        ...
        if self.training:
            ...
            if self.iou_aware:
                ...
                for i, out in enumerate(yolo_outputs):
                    ...
                    x = x.reshape((b, na, no, h * w))
                    ioup = ioup.reshape((b, na, 1, h * w))
                    obj = x[:, :, (2 + self.point_num * 2):(3 + self.point_num * 2), :]
                    ioup = F.sigmoid(ioup)
                    obj = F.sigmoid(obj)
                    obj_t = (obj**(1 - self.iou_aware_factor)) * (
                        ioup**self.iou_aware_factor)
                    obj_t = _de_sigmoid(obj_t)
                    loc_t = x[:, :, :(2 + self.point_num * 2), :]
                    cls_t = x[:, :, (3 + self.point_num * 2):, :]
                    y_t = paddle.concat([loc_t, obj_t, cls_t], axis=2)
                    y_t = y_t.reshape((b, c, h, w))
                    y.append(y_t)
                return y
