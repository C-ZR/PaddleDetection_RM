...
@register
@serializable
class PolyVOCDataSet(DetDataset):
    """
    ...
    def __init__(self,
                 ...
                 allow_empty=False,
                 empty_ratio=1.):
        super(PolyVOCDataSet, self).__init__(
    ...
    def parse_dataset(self, ):
        ...
        with open(anno_path, 'r') as fr:
            while True:
                ...
                gt_poly = np.zeros((num_bbox, 8), dtype=np.float32)
                ...
                for obj in objs:
                    ...
                    x1 = float(obj.find('gt_poly').find('x1').text)  # left
                    y1 = float(obj.find('gt_poly').find('y1').text)  # top
                    x2 = float(obj.find('gt_poly').find('x2').text)  # right
                    y2 = float(obj.find('gt_poly').find('y2').text)  # top
                    x3 = float(obj.find('gt_poly').find('x3').text)  # right
                    y3 = float(obj.find('gt_poly').find('y3').text)  # down
                    x4 = float(obj.find('gt_poly').find('x4').text)  # left
                    y4 = float(obj.find('gt_poly').find('y4').text)  # down

                    gt_poly[i, :] = [x1, y1, x2, y2, x3, y3, x4, y4]
