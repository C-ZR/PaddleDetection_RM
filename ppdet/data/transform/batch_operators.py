...
class Gt2YoloTarget(BaseOperator):
    ...
    def __call__(self, samples, context=None):
        ...
        for sample in samples:
            ...
            elif 'gt_poly' in sample:  # poly------------------------------------------------------------------------------------
                gt_poly = sample['gt_poly']
                gt_class = sample['gt_class']
                if 'gt_score' not in sample:
                    sample['gt_score'] = np.ones(
                        (gt_poly.shape[0], 1), dtype=np.float32)
                gt_score = sample['gt_score']
                for i, (
                        mask, downsample_ratio
                ) in enumerate(zip(self.anchor_masks, self.downsample_ratios)):
                    ...
                    grid_w = int(w / downsample_ratio)
                    target = np.zeros(
                        (len(mask), 4 + self.point_num * 2 + self.num_classes, grid_h, grid_w),
                        dtype=np.float32)
                    for b in range(gt_poly.shape[0]):
                        gx, gy, gw, gh, gw1, gh1, gw2, gh2, gw3, gh3, gw4, gh4 = gt_poly[b, :] # 归一化后的值
                        cls = gt_class[b]
                        score = gt_score[b]
                        if score <= 0.:
                            continue
                        ...
                        if best_idx in mask:
                            ...
                            # x, y, w1, h1, w2, h2, w3, h3, w4, h4
                            target[best_n, 0, gj, gi] = gx * grid_w - gi
                            target[best_n, 1, gj, gi] = gy * grid_h - gj

                            target[best_n,  2, gj, gi] = ((gw1 * w / self.anchors[best_idx][0]) / 4.0 + 0.5)
                            target[best_n,  3, gj, gi] = ((gh1 * h / self.anchors[best_idx][1]) / 4.0 + 0.5)
                            target[best_n,  4, gj, gi] = ((gw2 * w / self.anchors[best_idx][0]) / 4.0 + 0.5)
                            target[best_n,  5, gj, gi] = ((gh2 * h / self.anchors[best_idx][1]) / 4.0 + 0.5)
                            target[best_n,  6, gj, gi] = ((gw3 * w / self.anchors[best_idx][0]) / 4.0 + 0.5)
                            target[best_n,  7, gj, gi] = ((gh3 * h / self.anchors[best_idx][1]) / 4.0 + 0.5)
                            target[best_n,  8, gj, gi] = ((gw4 * w / self.anchors[best_idx][0]) / 4.0 + 0.5)
                            target[best_n,  9, gj, gi] = ((gh4 * h / self.anchors[best_idx][1]) / 4.0 + 0.5)

                            # scale
                            target[best_n, 10, gj, gi] = 2.0 - gw * gh

                            # objectness record gt_score
                            target[best_n, 11, gj, gi] = score

                            # classification
                            target[best_n, 12 + cls, gj, gi] = 1.
                        ...
                        if self.iou_thresh < 1:
                            for idx, mask_i in enumerate(mask):
                                ...
                                iou = jaccard_overlap(
                                    [0., 0., gw, gh],
                                    [0., 0., an_hw[mask_i, 0], an_hw[mask_i, 1]])
                                if iou > self.iou_thresh and target[idx, 11, gj,
                                                                    gi] == 0.:
                                    # x, y, w1, h1, w2, h2, w3, h3, w4, h4
                                    target[idx, 0, gj, gi] = gx * grid_w - gi
                                    target[idx, 1, gj, gi] = gy * grid_h - gj

                                    target[idx,  2, gj, gi] = ((gw1 * w / self.anchors[mask_i][0]) / 4.0 + 0.5)
                                    target[idx,  3, gj, gi] = ((gh1 * h / self.anchors[mask_i][1]) / 4.0 + 0.5)
                                    target[idx,  4, gj, gi] = ((gw2 * w / self.anchors[mask_i][0]) / 4.0 + 0.5)
                                    target[idx,  5, gj, gi] = ((gh2 * h / self.anchors[mask_i][1]) / 4.0 + 0.5)
                                    target[idx,  6, gj, gi] = ((gw3 * w / self.anchors[mask_i][0]) / 4.0 + 0.5)
                                    target[idx,  7, gj, gi] = ((gh3 * h / self.anchors[mask_i][1]) / 4.0 + 0.5)
                                    target[idx,  8, gj, gi] = ((gw4 * w / self.anchors[mask_i][0]) / 4.0 + 0.5)
                                    target[idx,  9, gj, gi] = ((gh4 * h / self.anchors[mask_i][1]) / 4.0 + 0.5)

                                    # scale
                                    target[idx, 10, gj, gi] = 2.0 - gw * gh

                                    # objectness record gt_score
                                    target[idx, 11, gj, gi] = score

                                    # classification
                                    target[idx, 12 + cls, gj, gi] = 1.
                    sample['target{}'.format(i)] = target
