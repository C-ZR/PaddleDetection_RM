...
class Resize(BaseOperator):
    ...
    def apply_segm(self, poly, scale, size):
        im_scale_x, im_scale_y = scale
        resize_w, resize_h = size
        poly[:, 0::2] *= im_scale_x
        poly[:, 1::2] *= im_scale_y
        poly[:, 0::2] = np.clip(poly[:, 0::2], 0, resize_w)
        poly[:, 1::2] = np.clip(poly[:, 1::2], 0, resize_h)
...
class Mixup(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        if 'gt_poly' in sample[0]:
            ...
            gt_poly = np.concatenate((gt_poly1, gt_poly2), axis=0)
...
class NormalizeBox(BaseOperator):
    ...
    def apply(self, sample, context):
        ...
        if 'gt_bbox' in sample:
            ...
            sample['gt_bbox'] = gt_bbox
        elif 'gt_poly' in sample:
            gt_poly = sample['gt_poly']
            height, width, _ = im.shape
            for i in range(gt_poly.shape[0]):
                gt_poly[i][0] = gt_poly[i][0] / width
                gt_poly[i][1] = gt_poly[i][1] / height
                gt_poly[i][2] = gt_poly[i][2] / width
                gt_poly[i][3] = gt_poly[i][3] / height
                gt_poly[i][4] = gt_poly[i][4] / width
                gt_poly[i][5] = gt_poly[i][5] / height
                gt_poly[i][6] = gt_poly[i][6] / width
                gt_poly[i][7] = gt_poly[i][7] / height
    ...
    def apply(self, sample, context=None):
        if 'gt_bbox' in sample:
            ...
            for b in range(poly.shape[0]):
                x1, y1, x2, y2, x3, y3, x4, y4 = poly[b, 0:8]
                gx = (x1 + x2 + x3 + x4) / 4.
                gy = (y1 + y2 + y3 + y4) / 4.
                xmin = min(x1, x2, x3, x4)
                ymin = min(y1, y2, y3, y4)
                gw = max(x1, x2, x3, x4) - xmin
                gh = max(y1, y2, y3, y4) - ymin
                gw1 = x1 - gx if x1!=gx or gx == 0 else 0.001
                gh1 = y1 - gy if y1!=gy or gy == 0 else 0.001
                gw2 = x2 - gx if x2!=gx or gx == 0 else 0.001
                gh2 = y2 - gy if y2!=gy or gy == 0 else 0.001
                gw3 = x3 - gx if x3!=gx or gx == 0 else 0.001
                gh3 = y3 - gy if y3!=gy or gy == 0 else 0.001
                gw4 = x4 - gx if x4!=gx or gx == 0 else 0.001
                gh4 = y4 - gy if y4!=gy or gy == 0 else 0.001
...
class PadBox(BaseOperator):
    ...
    def apply(self, sample, context=None):
        if 'gt_bbox' in sample:
            ...
            poly = sample['gt_poly']
            gt_num = min(self.num_max_boxes, len(poly))
            num_max = self.num_max_boxes
            # fields = context['fields'] if context else []
            pad_poly = np.zeros((num_max, 12), dtype=np.float32)
            if gt_num > 0:
                pad_poly[:gt_num, 0:8] = poly[:gt_num, 0:8]
            sample['gt_poly'] = pad_poly
            if 'gt_class' in sample:
                pad_class = np.zeros((num_max, ), dtype=np.int32)
                if gt_num > 0:
                    pad_class[:gt_num] = sample['gt_class'][:gt_num, 0]
