...
def calc_polybox_iou(pred, gt_poly):  # 测试------------------------------------------------------------------------------------------------------
    """
    calc iou between rotated bbox
    """
    x1, y1 = pred[0], pred[1]
    ...
    tx1, ty1 = gt_poly[0], gt_poly[1]
    tx2, ty2 = gt_poly[2], gt_poly[3]
    tx3, ty3 = gt_poly[4], gt_poly[5]
    tx4, ty4 = gt_poly[6], gt_poly[7]
    ...
    xkis1 = max(xmin, txmin)
    ykis1 = max(ymin, tymin)
    xkis2 = min(xmax, txmax)
    ykis2 = min(ymax, tymax)
    w_inter = xkis2 - xkis1
    h_inter = ykis2 - ykis1
    overlap = w_inter * h_inter

    area1 = (xmax - xmin) * (ymax - ymin)
    area2 = (txmax - txmin) * (tymax - tymin)
            ...
            for i, gl in enumerate(gt_label):
                if int(gl) == int(l):
                    if len(gt_box[i]) == 5:
                        ...
                        overlap = calc_polybox_iou(pred, gt_box[i])
                    else:
